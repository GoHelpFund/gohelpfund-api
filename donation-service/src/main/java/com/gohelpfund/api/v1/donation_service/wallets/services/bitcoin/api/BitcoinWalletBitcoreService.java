...
import com.gohelpfund.api.v1.donation_service.wallets.clients.BitcoinBitcoreRestTemplateClient;
...
@Service
public class BitcoinWalletBitcoreService {
    private static final Logger logger = LoggerFactory.getLogger(BitcoinWalletBitcoreService.class);
    ...
    @Autowired
    BitcoinBitcoreRestTemplateClient bitcoinBitcoreClient;

    public BitcoreAddr getInsightBitcoinAddr(String bitcoinAddress) {
        BitcoreAddr bitcoreAddr = bitcoinBitcoreClient.getBitcoreBitcoinAddrBalance(bitcoinAddress);

        if (bitcoreAddr != null) {
            logger.debug("GET | /api.bitcore.io/api/BTC/addr/{id} | found | bitcoin address: {}", bitcoinAddress);
        } else {
            logger.debug("GET | /api.bitcore.io/api/BTC/addr/{id} | not found | bitcoin address: {}", bitcoinAddress);
        }
        return bitcoreAddr;
    }

    private InsightTxResponse setInsightBitcoinTx(String rawtxid) {
        InsightTxResponse insightBitcoinTx = bitcoinBitcoreClient.setTransaction(getHttpEntity(rawtxid));
